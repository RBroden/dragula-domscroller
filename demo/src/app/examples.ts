...
@Component({
  selector: 'my-example',
  templateUrl: './templates/my-example.html',
  styleUrls: ['./styles/my-example.css']
})
export class MyExample implements AfterViewInit {
  @ViewChild('myContainer') myContainer: any;
  count: number = 15;
  data: number[] = ( () => {
    let numbers = [];
  ...
  ngAfterViewInit(): void {
    this.scroll = autoScroll([
      this.myContainer.nativeElement
    ], {
      margin: 20,
      maxSpeed: 5,
      scrollWhenOutside: true,
      autoScroll: () => {
        // works but is always working; not just when dragging
        //return true;
